...
from models import Optimizer_mul, hrlcdr
from myutils import *
...
cell_sim_f = calculate_gene_exponent_similarity7(
    x=torch.from_numpy(exprs).to(dtype=torch.float32, device=device), mu=3
)
drug_sim_f = jaccard_coef7(
    tensor=torch.from_numpy(drug_finger).to(dtype=torch.float32, device=device)
)
...
# This doesn't work for imbalance dataset.
...
for n_kfold in range(n_kfolds):
    for train_index, val_index in kfold.split(a):
        ...
        true_datas = pd.concat(
            [true_datas, translate_result(true_data)], ignore_index=True
        )
        predict_datas = pd.concat(
            [predict_datas, translate_result(best_predict)], ignore_index=True
        )
...
print(len(auprs))
logging.info(
    f"final_auc:{np.mean(aucs):.4f},var:{np.var(aucs)},final_aupr:{np.mean(auprs):.4f},var:{np.var(auprs)}"
)
