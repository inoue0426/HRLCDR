...
from models import hrlcdr_ic50, Optimizer_mul_ic50
...
for n_kfold in range(n_kfolds):
    for train_index, val_index in kfold.split(a):
        ...
        model = hrlcdr_ic50(adj_mat=sampler.train_data,  cell_exprs=exprs, drug_finger=drug_finger,
                        gamma=args.gamma, drug_hyper = A6, cell_hyper = B4,device=args.device).to(args.device)
...
cv_set.to_csv("./HRLCDR/GDSC/Data/5-fold_CV.csv",index=False)
test_set.to_csv("./HRLCDR/GDSC/Data/testset.csv",index=False)

pd.DataFrame(true_datas).to_csv("./HRLCDR/GDSC/result_data/true_data.csv")
pd.DataFrame(predict_datas).to_csv("./HRLCDR/GDSC/result_data/predict_data.csv")
