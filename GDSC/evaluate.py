...
data_dir = "./HRLCDR/GDSC/result_data/"
auc_all = []
ap_all = []
acc_all = []
f1_all = []
...
for i in np.arange(25):
    predict_data = pd.read_csv(
        data_dir + "predict_data.csv", index_col=0, header=0, nrows=1, skiprows=i
    )
    ...
    true_data = pd.read_csv(
        data_dir + "true_data.csv", index_col=0, header=0, nrows=1, skiprows=i
    )
...
file_write_obj.writelines(
    "|final auc: {}|final ap: {}|final acc: {}|final f1: {}|final mcc: {}".format(
        str(np.mean(auc_all)),
        str(np.mean(ap_all)),
        str(np.mean(acc_all)),
        str(np.mean(f1_all)),
...
file_write_obj.writelines(
    "|final auc_var: {}|final ap_var: {}|final acc_var: {}|final f1_var: {}|final mcc_var: {}".format(
        str(np.var(auc_all)),
        str(np.var(ap_all)),
        str(np.var(acc_all)),
        str(np.var(f1_all)),
