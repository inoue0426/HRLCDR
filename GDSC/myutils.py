...
def hyper_graph(sima, guanlian, simb):
    ...
    A3 = torch.mul(torch.mm(torch.mm(guanlian,simb),guanlian.T), sima) + torch.mul(torch.mm(torch.mm(guanlian,simb),guanlian.T), sima).T
...
def creat_G_and_X_and_models_prarams(kegg, PPI, cell_exp):
    ...
    X = []
    models_params = []
    pathway_contain = pd.DataFrame(np.zeros((34,2369)),index=kegg.keys(),columns=PPI.columns)
    for key, _ in kegg.items():
        ...
        models_params.append({"input_size": cell.shape[0], "hidden_size": cell.shape[0], "output_size": 30})
...
def pcc_score(true_data: torch.Tensor, predict_data: torch.Tensor):
    ...
    :return: pcc score
    """
    pcc = pearsonr(true_data.detach().cpu().numpy(), predict_data.detach().cpu().numpy())[0]
    return pcc
...
def huber_loss(true_data: torch.Tensor, predict_data:  torch.Tensor, masked: torch.Tensor,hu):
    """
    :param true_data: true data
    :param predict_data: predict data
    :param masked: data mask
...
def ji_loss(true_data: torch.Tensor, predict_data:  torch.Tensor, masked: torch.Tensor,
            cell_emb, drug_emb,cell_sim,drug_sim):
    masked = masked.to(torch.bool)
    true_data = torch.masked_select(true_data, masked)
    predict_data = torch.masked_select(predict_data, masked)
    loss_fun = nn.BCELoss()
...
def cross_entropy_loss(true_data: torch.Tensor, predict_data:  torch.Tensor, masked: torch.Tensor):
    ...
    loss = loss_fun(predict_data, true_data)
    return loss
def re_entropy_loss(predict_data:  torch.Tensor,true_data: torch.Tensor, masked: torch.Tensor):
    """
    :param true_data: true data
    :param predict_data: predict data
    :param masked: data mask
...
def sc_score(true_data: torch.Tensor, predict_data: torch.Tensor):
    ...
    """
    sc = spearmanr(true_data.detach().cpu().numpy(), predict_data.detach().cpu().numpy())[0]
    return sc
...
def rmse_score(true_data: torch.Tensor, predict_data: torch.Tensor):
    ...
    :return: RMSE score
    """
    rmse = np.sqrt(mean_squared_error(true_data.detach().cpu().numpy(), predict_data.detach().cpu().numpy()))
    return rmse
