...
from myutils import *
from HRLCDR.CCLE.HRLCDR_New import HRLCDR_new
import copy
import pandas as pd
import numpy as np
import pandas as pd
...
parser.add_argument('--tol', type=float,default=20,
...
cell_sim_fo = calculate_gene_exponent_similarity7(x=torch.from_numpy(exprs).to(dtype=torch.float32, device='cuda:0'), mu=3)
drug_sim_fo = jaccard_coef7(tensor=torch.from_numpy(drug_finger).to(dtype=torch.float32, device='cuda:0'))
...
#====================================dataset=========================================
sed = 1
kf = KFold(n_splits=5, shuffle=True,random_state=sed)
kfold = KFold(n_splits=5, shuffle=True,random_state=sed)
x, y = cell_drug.shape
...
for train_index, test_index in kf.split(np.arange(y)):
...
#==========================================================================
    ...
    for train_ind, test_ind in kfold.split(np.arange(sum(sum(new_A)))):
        ...
        auc,aupr,true_data, predict_data = HRLCDR_new(cell_exprs=exprs,drug_finger=drug_finger,
                                                            sam=sam,test_da=cell_drug,test_ma=banlence_mask,
                                                            args=args,cell_hyper=B4,drug_hyper=A6,
                                        A1 = A1, A2 = A2, A3 = A3, B1 = B1, B2 = B2, B3  = B3,
                device=args.device,alp=0.85,bet=0.15)
...
logging.info(f"final_auc:{np.mean(final_auc_d):.4f},var:{np.var(final_auc_d)},final_aupr:{np.mean(final_aupr_d):.4f},var:{np.var(final_aupr_d)}")
