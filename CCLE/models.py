...
class Hyper_Agg(nn.Module):
    def __init__(
        self, adj_mat, adj_mat_T, self_c, self_d, cell_hyper, drug_hyper, alp, bet
    ):
        ...
        self.chaotuHC = torch.eye(436).to("cuda:0") - torch.mm(
            torch.mm(B4H, cell_hyper), B4H
        )
        self.chaotuHD = torch.eye(24).to("cuda:0") - torch.mm(
