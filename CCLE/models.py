...
class Hyper_Agg(nn.Module):
    def __init__(self, adj_mat,adj_mat_T,self_c,self_d, cell_hyper,drug_hyper,
                 alp,bet):
        ...
        self.cell_sim = cell_hyper
        self.drug_sim = drug_hyper
        ...
        self.chaotuHC = torch.eye(436).to("cuda:0") - torch.mm(torch.mm(B4H, cell_hyper), B4H)
        self.chaotuHD = torch.eye(24).to("cuda:0") - torch.mm(torch.mm(A4H, drug_hyper), A4H)
    ...
    def forward(self,exp,fig):
        ...
        cell4 = self.agg_b4(torch.mm(self.cell_sim, exp))
        ...
        drug4 = self.agg_a4(torch.mm(self.drug_sim, fig))
...
class hrlcdr(nn.Module, ABC):
    def __init__(self, adj_mat, cell_exprs, drug_finger,  gamma,
        ...
        self.agg_h = Hyper_Agg(adj_mat=agg_cell_lp,adj_mat_T=agg_drug_lp,self_c=self_cell_lp,self_d=self_drug_lp,
                                        cell_hyper=cell_hyper,drug_hyper=drug_hyper,
                                        alp=1,bet=1).to(device)
...
class hrlcdr_new(nn.Module, ABC):
    def __init__(self, adj_mat, cell_exprs, drug_finger,  gamma,drug_hyper, cell_hyper,
        ...
        self.ldd = nn.Linear(self.figprint.shape[1],2040,bias=True).to(device)
        self.lcc = nn.Linear(self.cexp.shape[1],2040,bias=True).to(device)

        self.agg_h = Hyper_Agg(adj_mat=agg_cell_lp,adj_mat_T=agg_drug_lp,self_c=self_cell_lp,self_d=self_drug_lp,
                                        cell_hyper=cell_hyper,drug_hyper=drug_hyper,alp=alp,bet=bet).to(device)
        self.decoder = GDecoder(gamma=gamma)
...
class Optimizer_new(nn.Module):
    def __init__(self, model,  train_data, test_data, test_mask, train_mask,ind_data,ind_mask,
                 ...
                 lr=0.001, wd=1e-05, epochs=200, tol=20,test_freq=20, device="cpu"):#
